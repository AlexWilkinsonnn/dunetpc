////////////////////////////////////////////////////////////////////////
// Class:       LoadFDSimChannelsTrim
// Plugin Type: producer (Unknown Unknown)
// File:        LoadFDSimChannelsTrim_module.cc
//
...
class extrapolation::LoadFDSimChannelsTrim : public art::EDProducer {
public:
  explicit LoadFDSimChannelsTrim(fhicl::ParameterSet const& p);
  ...
  LoadFDSimChannelsTrim(LoadFDSimChannelsTrim const&) = delete;
  LoadFDSimChannelsTrim(LoadFDSimChannelsTrim&&) = delete;
  LoadFDSimChannelsTrim& operator=(LoadFDSimChannelsTrim const&) = delete;
  LoadFDSimChannelsTrim& operator=(LoadFDSimChannelsTrim&&) = delete;
...
extrapolation::LoadFDSimChannelsTrim::LoadFDSimChannelsTrim(fhicl::ParameterSet const& p)
{
...
void extrapolation::LoadFDSimChannelsTrim::produce(art::Event& e)
{
  if (!fEventIDs.size()) {
    throw cet::exception("LoadFDSimChannelsTrim")
...
void extrapolation::LoadFDSimChannelsTrim::beginJob()
{
  std::cout << "Readng data from " << fNDFDH5FileLoc << "\n";
  fFile = new HighFive::File(fNDFDH5FileLoc, HighFive::File::ReadOnly);

  // Read in data
  for (const std::string eventIDStr : fFile->getGroup("fd_simchannels_trim").listObjectNames()) {
    const int eventID = std::stoi(eventIDStr);
    const std::string eventGroupPath = "fd_simchannels_trim/" + eventIDStr;
...
DEFINE_ART_MODULE(extrapolation::LoadFDSimChannelsTrim)
